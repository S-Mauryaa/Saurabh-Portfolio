export default async (req, context) => {
  ...
  if (!platform || !username) {
    return new Response(JSON.stringify({ error: 'Platform and username are required' }), { status: 400, headers });
  }
  ...
  try {
    ...
    if (data) {
      ...
      return new Response(JSON.stringify({ error: `Could not fetch stats for ${platform} user: ${username}` }), { status: 404, headers });
    }
  } catch (err) {
    console.error('fetch-coding-stats error:', err);
    return new Response(JSON.stringify({ error: 'Failed to fetch coding stats' }), { status: 500, headers });
  }
};

// Helper functions (same as in server.js/api version)
